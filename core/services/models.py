...
class Work(models.Model):
    ...
    credit_cleared = models.BooleanField(default=False)
    credit_cleared_at = models.DateTimeField(null=True, blank=True)
    ...
    def get_remaining_balance(self):
        ...
        remaining = self.price - self.get_total_payments()
    ...
    def get_days_outstanding(self):
        """Get number of days work has been pending (incomplete)"""
        if self.completed:
            return 0
        from django.utils import timezone
    ...
    def is_overdue(self, days_threshold=2):
        """Check if incomplete work is overdue (older than threshold days)"""
        if self.completed:
            return False
        return self.get_days_outstanding() >= days_threshold
...
class DailySalesReport(models.Model):
    ...
    approved_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name='sales_reports_approved'
    )
    ...
    def can_be_edited_by(self, user):
        ...
        if user.is_admin:
            return True
        if self.approval_status == self.STATUS_DRAFT and self.generated_by == user:
            return True
        return False
    
    def submit_report(self):
        """Submit the report (locks it for non-admin users)"""
        if self.approval_status == self.STATUS_DRAFT:
    ...
    def approve_report(self, approver):
        """Approve a previously submitted report."""
        if self.approval_status != self.STATUS_PENDING_APPROVAL:
            return False
...
class PendingStockAdjustment(models.Model):
    """Staff-submitted stock additions and material creations pending admin approval"""
    ...
    adjustment_type = models.CharField(
        ...
        help_text='Type of adjustment: stock addition to existing material or creation of new material'
    )
    ...
    material = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text='Existing material (for stock_addition type)'
    )

    # For new material submissions
    material_name = models.CharField(
        max_length=255,
        blank=True,
        null=True,
        help_text='Name for new material (for new_material type)'
    )
    material_category = models.CharField(
        max_length=50,
        choices=Material.CATEGORY_CHOICES,
        blank=True,
        null=True,
        help_text='Category for new material (for new_material type)'
    )
    material_unit = models.CharField(
        max_length=50,
        blank=True,
        null=True,
        help_text='Unit of measure for new material (for new_material type)'
    )
    reorder_level = models.IntegerField(
        default=0,
        help_text='Reorder level for new material (for new_material type)'
    )

    quantity = models.IntegerField(help_text='Quantity to add to stock')
    reason = models.TextField(help_text='Reason for adding stock or creating material')
    ...
    def __str__(self):
        if self.adjustment_type == 'new_material':
            return f"New {self.material_name} ({self.status})"
    ...
    def get_display_name(self):
        ...
        return self.material.name if self.material else 'Unknown Material'
